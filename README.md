...
2. From there, fetch the code from the GitHub repository with
   ...
   /NuOscProbExact/pyproject.toml      Packaging metadata, so the code can be pip-installed
   ...
   /NuOscProbExact/tests               Contains the regression test suite, run with pytest
       ../test_su3_algebra.py          Checks the d tensor, star product, and SU(3) invariants
       ../test_evolution_operator.py   Checks U against an independent matrix exponential
       ../test_probabilities.py        Checks normalization, positivity, and P = |U|^2
       ../test_hamiltonians.py         Checks the sample Hamiltonians and their sign conventions
       ../test_reference_formulas.py   Checks the exact result against the standard formulas
       ../test_edge_cases.py           Checks degenerate and near-degenerate Hamiltonians
