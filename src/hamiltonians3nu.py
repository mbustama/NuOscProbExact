...
def hamiltonian_vacuum_energy_independent(s12, s23, s13, dCP, D21, D31,
...
def hamiltonian_matter(h_vacuum, A):
...
def hamiltonian_nsi(h_vacuum, A, eps):
    ...
    # Hamiltonian
    h_nsi[0][0] += A*(1.0+eps_ee)
    h_nsi[0][1] += A*eps_em
    h_nsi[0][2] += A*eps_et
    h_nsi[1][0] += A*np.conj(eps_em)
    h_nsi[1][1] += A*eps_mm
    h_nsi[1][2] += A*eps_mt
    h_nsi[2][0] += A*np.conj(eps_et)
    h_nsi[2][1] += A*np.conj(eps_mt)
    h_nsi[2][2] += A*eps_tt
