...
def pmns_mixing_matrix(
    ...
    .. versionchanged:: 1.1.0
       The entries are complex throughout.  They were previously a
       mixture of complex and real numbers, depending on which of them
       the CP phase reached.  The matrix is still returned as a nested
...
def hamiltonian_3nu_vacuum_energy_independent(
    ...
    .. versionchanged:: 1.4.0
       Faster, with identical results --- all 42 figures generated by
       ``run_testsuite.py`` are byte-for-byte those of 1.3.0. The closed
       form rebuilt the CP phase fifteen times across its nine entries
       and recomputed two products in five places; hoisting them makes
       the routine 1.9x quicker.
    ...
    c13 = _cos_from_sin(s13, 's13', caller)
...
def hamiltonian_3nu_liv(
    ...
    .. versionchanged:: 1.4.0
       Faster, with identical results --- all 42 figures generated by
       ``run_testsuite.py`` are byte-for-byte those of 1.3.0. The gain
       is indirect, through `pmns_mixing_matrix`, which this routine
       builds the rotation from.
