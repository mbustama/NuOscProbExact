...
\end{verbatim}
It depends on {\tt NumPy} and {\tt Numba}, so the compiled backend comes with it.
...
\ref{app:sample_2nu_hamiltonians} shows the two- and four-flavor counterparts of the scenarios below.  We implement them all in {\tt NuOscProbExact}\ \cite{NuOscProbExact}.  
...
{\tt nuCraft}'s sterile and charged-current matter entries are set by two independently rounded literals whose ratio is $0.5016$, where the isoscalar value $(1/2)(1 - Y_e)/Y_e$ at $Y_e = 0.5$ is exactly $1/2$.  That is a property of its constants rather than of its solver.  Scored against a reference that does not share that ratio, no setting removes the resulting $0.3$ percent offset, because rescaling the density scales both entries together.  We score the curve drawn here in {\tt nuCraft}'s own constants, as every curve in the figure is scored in its own.  We report the discrepancy here rather than charge it to its solver.
