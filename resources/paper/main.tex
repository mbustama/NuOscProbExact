...
This performance axis is the one {\tt NuOscProbExact} has and the four external codes in \figu{performance} do not, for a reason about their interfaces rather than their arithmetic.  {\tt GLoBES}, {\tt Prob3++}, and {\tt NuFast-LBL} take one energy per call, and {\tt nuSQuIDS} exposes a multiple-energy mode.  Its cost per probability does not fall as the request grows: at tens of thousands of energies it is about twice what it is at a handful.  Two of the curves in \figu{performance} do fall at the largest stacks, both by about a factor of two against their own plateaus: {\tt NuFast-LBL} from $72$ to $35$~ns, and {\tt Prob3++} from $1310$ to $671$~ns.  Since neither batches, that is not amortization; {\tt GLoBES}, timed the same way on the same machine, shows no such fall, so it is not an artifact of the timing either; we have not established what it is.  Regardless, at a factor of two against the two orders of magnitude batching gives, it alters no conclusion that rests on this figure.  These curves are not a comparison: each code is timed at a setting of its own.  Section \ref{sec:comparison} holds speed against accuracy.
...
The six codes that can cross the Earth treat the varying profile in one of two ways.  {\tt NuOscProbExact}, {\tt NuFast-Earth}, {\tt GLoBES}, and {\tt Prob3++} cut the path into pieces of constant density and solve each piece exactly.  Between one boundary and the next, nothing is approximated.  {\tt nuSQuIDS} and {\tt nuCraft} instead integrate the equations of motion numerically along the trajectory.  The treatment of the profile does not, however, fix what refinement can reach.
...
\begin{figure*}[t!]
 ...
 \caption{\label{fig:planes}What accuracy costs, in three problems that ask progressively more of a code.  All three share one axis of time per probability, so that a cost can be carried by eye from one panel to the next.  {\it Top}: $P_{\nu_\mu \to \nu_e}$ at $L = 1300$~km through constant matter of $3$~g~cm$^{-3}$, largest error over $60$ energies between $0.6$ and $20$~GeV.  Nothing is approximated there, so the reference is exact: the same Hamiltonian exponentiated in $50$-digit arithmetic.  {\it Middle}: $P_{\nu_\mu \to \nu_\mu}$ along the PREM chord at $\cos\theta_z = -0.9$, a baseline of $11468$~km, over twelve energies between $3$ and $40$~GeV.  {\it Bottom}: the same chord with a sterile state, at the mixing of \figu{sterile_earth}, $\Delta m_{41}^2 = 1$~eV$^2$ and $\sin^2\theta_{14} = \sin^2\theta_{24} = 0.1$, over twelve energies between $0.3$ and $30$~TeV; {\tt NuFast-Earth}, {\tt GLoBES}, and {\tt Prob3++} hard-wire three flavors and cannot run it.  In the two lower panels the density varies along the chord and every code, this one included, must approximate the profile, so no exact answer exists and the reference is instead a converged one, described in the text.  {\tt NuOscProbExact} appears twice in the lower two panels, once for each dial it exposes, and {\tt NuFast-Earth} twice for a different reason; the text gives both, along with the constants each code is allowed to keep.  Lower and further left is better.}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
{\tt NuOscProbExact} gives up speed alone on this problem.  It is the slowest of the four exact entries, by a factor of $17$ against {\tt NuFast-LBL} and about $3$ against {\tt GLoBES} and {\tt Prob3++}, while in accuracy the four are separated only by where each one's round-off happens to fall.  That is the price of generality: the other three solve the standard three-flavor case and no other, while the same code path here takes any Hermitian Hamiltonian at two, three, or four flavors.  Much of the gap is the fixed cost of entering the compiled kernel, so it narrows with the size of the request, which \figu{performance} measures.
...
{\tt NuFast-Earth} caches the work that depends on the profile, so what it costs depends on which parameter a scan moves.  A scan over $\delta_{\rm CP}$ invalidates none of that work, and costs $0.06$~$\mu$s per probability however finely the Earth is cut---three decades to the left of anything else on the plane, and a measure of one cache rather than of a propagation.  Moving $\Delta m^2_{31}$ instead invalidates it.  The same code then costs $4000$~$\mu$s.  Both are drawn; the unqualified name goes to the second, since that is the cost when a fit moves a parameter the cache does not cover.  No other code here needs the pair: on this grid the ratio between the two scans is $1.02$ for {\tt nuSQuIDS}, $1.01$ for {\tt Prob3++} and {\tt nuCraft}, $1.00$ for {\tt GLoBES}, and $0.95$ for this code.
...
\begin{tabular}{l c c c >{\raggedright\arraybackslash}p{3.2cm} c c >{\raggedright\arraybackslash}p{3.0cm}}
 ...
 {\tt NuFast-LBL}\ \cite{Denton:2024pzc}
   & $2.0.1$ & {\tt C++} & $3$ & Standard & no & Newton steps
   & Long-baseline fits \\
