...
\begin{equation}
 ...
 \right) ,
\end{equation}
which is the standard parametrization of \Ref\ \cite{Tanabashi:2018oca}.  The routines take $\sin\theta_{ij}$ rather than $\theta_{ij}$, which is the other convention to state, and global fits publish $\sin^2\theta_{ij}$ instead, as the values below are quoted: passing a published number where a sine is expected is a silent error rather than a loud one, since $0.310$ is a perfectly good sine of a different angle.  Every routine that takes an angle therefore takes an {\tt angles} argument --- {\tt 'sin'} by default, or {\tt 'sin2'}, {\tt 'rad'} or {\tt 'deg'} --- so that a published parameter set can be given in the form it was published in.  The last two carry $\delta_{\rm CP}$ with them; the first two leave it in radians, a phase having no sine to pass, since $\sin\delta_{\rm CP}$ would lose its quadrant.  One piece of bookkeeping goes with them, since Listing\ \ref{lst:3nu_examples} relies on it: what {\tt hamiltonian\_3nu\_vacuum\_energy\_independent} returns is $\tfrac{1}{2} \mathbb{R}_{3,\theta} \mathbb{M}_3^2 \mathbb{R}_{3,\theta}^\dagger$, with the factor of $\tfrac{1}{2}$ in \equ{h3_vacuum} already folded in, which is why the listing divides it by $E$ and not by $2E$.
...
Listing\ \ref{lst:antineutrinos} shows both routes: the flag, and the two operations written out for a Hamiltonian of one's own.  The cost of applying only one of them is not subtle.  At $L = 1300$~km in the crust and $E = 2.5$~GeV, $P_{\nu_\mu \to \nu_e}$ is $0.087$ for neutrinos and $0.024$ for antineutrinos; conjugating the vacuum term but leaving the potential alone gives $0.069$, which is neither, and is wrong by a factor of three.  Notebook 13 in the repository works through both ways of getting it half right.
...
# Notebook 20 builds it; zeros here so that the snippet
# runs as printed.  Any Hermitian term would do.
v_emu = np.zeros(len(widths))
q = np.diag([1., -1., 0.])
...
\figu{lri_earth} shows the result for a neutrino crossing the full diameter of the Earth.  The two curves are the same PREM slab decomposition, evaluated with and without the extra term, and the only difference between the calculations that produced them is the single line that adds $V_{e\mu}$ to the matter Hamiltonian.  At $\alpha^\prime \equiv g^{\prime 2}/4\pi = 10^{-52}$, and a mediator of range $1/m_{Z^\prime} = R_\oplus$, the potential at the center of the Earth is about $1\%$ of $V_{\rm CC}$ there, and the appearance probability is displaced by up to $0.10$ in absolute terms, with the largest effect near $4$~GeV, where the standard matter resonance sits.  Notebook 20 fixes the normalization of $V_{e\mu}$ and is where these numbers come from; it is the one figure here whose input the listings do not carry in full.  The lower panel is that difference on its own scale.  The point of the figure is not the parameter value, which is chosen to make the effect visible rather than to bound anything; it is that a scenario none of the codes of Section \ref{sec:comparison} anticipates costs the user one array and one addition.  Notebook 20 in the repository builds $V_{e\mu}$ from scratch, checks it against its short- and long-range limits, and scans $\alpha^\prime$ over three decades.
