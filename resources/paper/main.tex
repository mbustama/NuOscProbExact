...
One safeguard does fire on its own, guarding against refinement rather than length.  Given a tolerance in place of a slab count, the routines double the count until the estimated error meets it.  A tolerance finer than the discretization can reach would go on doubling until memory ran out, so they stop at $1024$ sub-slabs per segment and raise an exception that names the error reached.  \textcolor{blue}{The ceiling moves with {\tt n\_max}, but only so far: every sub-slab contributes its own round-off to the product, so past the point where that overtakes the discretization error, refining returns a worse answer.  The exception distinguishes the two cases and names the finest result it reached, rather than asking for a larger ceiling in a regime where none would help.}
...
{\tt NuFast-Earth} caches the work that depends on the profile, so what it costs depends on which parameter a scan moves.  A scan over $\delta_{\rm CP}$ invalidates none of that work, and costs $0.06$~$\mu$s per probability however finely the Earth is cut---three decades to the left of anything else on the plane at the same accuracy, and a measure of one cache rather than of a propagation.  Moving $\Delta m^2_{31}$ instead invalidates it.  The same code then costs $4000$~$\mu$s.  The cheap path is not peculiar to $\delta_{\rm CP}$: by its design\ \cite{Denton:2025oxf} any parameter that leaves the layer eigenvalues alone takes it, $\theta_{23}$ among them, as does the production height that atmospheric analyses integrate over---we measured the $\delta_{\rm CP}$ scan.
