...
We draw both; the unqualified name goes to the second, since that is the cost when a fit moves a parameter the cache does not cover.  No other code here shows anything like it.  On this grid the ratio between the two scans is $1.2$ for {\tt nuCraft} and $1.1$ for {\tt nuSQuIDS}, and within a few percent of one for {\tt Prob3++}, {\tt GLoBES} and this code, against the factor of some $10^5$ between {\tt NuFast-Earth}'s two scans.  Each of those ratios is a single measurement against a block-to-block scatter of about a tenth, so the two that sit above one are not resolved from it.
