...
Three properties, hard to come by together, recommend {\tt NuOscProbExact}.  It is \emph{flexible}: any Hermitian Hamiltonian at two, three, or four flavors goes through the same call, so what a probability costs is fixed by the size of the matrix and never by the physics inside it.  It is \emph{fast}: for calculation of oscillation probabilities of neutrinos going through the Earth it is the quickest of the codes we compare, from the coarsest setting they share down to an accuracy of $10^{-7}$; past that, one general-purpose solver reaches finer answers in less time.  And it is \emph{exact}: handed a Hamiltonian, it returns the probability to round-off.  Three of the codes we compare do the same at constant matter density, each for the standard three-flavor case alone; this one does it for every Hamiltonian in scope.  Through the Earth it is overtaken only at the finest settings, and by a general-purpose solver rather than by a specialized one; at constant density the closed forms written for three flavors are faster (Section \ref{sec:comparison}).  That combination suits the exploration of non-standard scenarios, where broad parameter spaces must be scanned without knowing \textit{a priori} which regions matter, and without waiting for a tailored approximation to be derived.
...
The two integrators appeared to stop at a floor that no solver setting passed.  For {\tt nuSQuIDS} the same value survived five steppers, every maximum step size, and every tolerance below $10^{-9}$, so the natural reading was that the PREM's density jumps held them there.  That reading was wrong.  The floor lay in the comparison rather than in either code.
...
Set beside existing oscillation codes (Section \ref{sec:comparison}), {\tt NuOscProbExact} makes good on three claims: flexibility, speed, and accuracy.  The call accepts the Hamiltonian itself, any Hermitian matrix, rather than a list of oscillation parameters, so a non-standard interaction, a Lorentz-violating term, an added sterile state, all three at once, or a completely novel oscillation scenario is the same computation as the standard case.  Among the six codes we examined that cross the Earth, {\tt NuOscProbExact} is the quickest route to every accuracy down to about $10^{-7}$, past which {\tt nuSQuIDS} is both finer and faster (the middle panel of \figu{planes}).  A scan requested as one call comes out two orders of magnitude cheaper per probability than the same scan run point by point (\figu{performance}).  At constant density its answer differs from a $50$-digit reference by round-off alone, as do those of the other exact codes there, which are faster (the top panel of \figu{planes}); no accuracy setting exists there because no approximation is made for one to refine.
...
Table \ref{tab:tests} lists what the 904 tests of {\tt NuOscProbExact} cover; \ref{app:structure} shows the modules they exercise.  They reach every line and every branch of the library: coverage is $100\%$ in v1.13.1, held above a floor of $98\%$ that fails the build if it drops.
