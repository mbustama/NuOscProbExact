...
\end{enumerate}
Three further assumptions sit beneath the probability itself rather than beneath the method, and are shared with every calculation in this paper.  The neutrino is ultrarelativistic, which Section \ref{sec:general_remarks} uses to set $L \simeq t$; it propagates as a plane wave rather than as a wave packet; and the same flavor basis serves at production and at detection.  Both conditions above hold in many scenarios studied in the literature---oscillations in vacuum, in matter of constant density, and in diverse new-physics settings---but not in others, notably propagation through the Earth.  For those, we extend the method to Hamiltonians that vary along the path, by treating the density profile as piecewise constant: the approximation is in the profile, never in the propagation through it.
...
Equation (\ref{equ:sylvester}) is one object, and what separates the exact methods in the oscillation literature is which part of it each keeps, and in which basis.  Contracting it with the trace metric, $u_0 = \frac{1}{n}{\rm Tr}\,\mathbb{U}$ and $u_a = -\frac{i}{2}{\rm Tr}(\lambda^a \mathbb{U})$---in the convention $\mathbb{U} = u_0 \mathbb{1} + i u_a \lambda^a$ used throughout, with generators normalized so that ${\rm Tr}\left(\lambda^a \lambda^b\right) = 2\delta^{ab}$---keeps the whole matrix in the generator basis, which is the form of \Ref\ \cite{Merfeld:2014cha} and the one implemented here.  Seen this way, the OS coefficients are the SU($n$)-basis components of the adjugate, and another value of $n$ requires no new idea.
...
\end{eqnarray}
where the contractions are how {\tt NuOscProbExact} evaluates the two invariants, and the trace forms are the ones Section \ref{sec:four_flavors} carries over for four flavors.
...
Sections \ref{sec:two_flavors} and \ref{sec:three_flavors} are \equ{sylvester} at $n=2$ and $n=3$; the expansions there, and the closed forms built on them, are due to OS.  The construction goes through unchanged at $n=4$, the value their own remark identifies as reachable without carrying it out (see Section \ref{sec:scope}), and the last at which the eigenvalues can be written in radicals.
...
A compiled backend based on the {\tt Numba} just-in-time compiler, and installed with the code, gives a further factor of eight to thirteen on top of that, and agrees with the pure-{\tt NumPy} path to round-off.  It is used only above the stack sizes where it has been measured to be faster: every stack at three and four flavors, and only large ones at two.  Section \ref{sec:performance} quantifies all of this.  By default, the code also checks that the Hamiltonian it is given is Hermitian, and raises an error if it is not, rather than silently returning quantities that still sum to one.
...
The method assumes what Section \ref{sec:introduction} states: a closed system, and a Hamiltonian constant over each interval that \equ{sylvester} is applied to (which, by \equ{composition}, need not be the whole path).
...
\begin{figure}[t!]
 ...
 \caption{\label{fig:prob_3nu}Three-neutrino oscillation probabilities $P_{\nu_e \to \nu_e}$ ({\it top}), $P_{\nu_\mu \to \nu_e}$ ({\it center}), and $P_{\nu_\mu \to \nu_\mu}$ ({\it bottom}), computed using {\tt NuOscProbExact}\ \cite{NuOscProbExact}.  The scenarios shown are for oscillations in vacuum, in matter of constant density, with non-standard interactions (NSI), and in a CPT-even Lorentz-violating background (LIV).  In all cases, the baseline is $L = 1300$~km.  See the main text for details. \vspace*{-0.5cm}}
 \vspace*{0.03cm}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Oscillations in matter might receive sub-leading contributions due to new neutrino interactions with the fermions of the medium through which they propagate.  These are known as non-standard interactions (NSI); see \Refs\ \cite{Ohlsson:2012kf, Miranda:2015dra, Farzan:2017xzy, Esteban:2018ppq} for reviews.  
...
A 3+1 system carries three Dirac phases rather than one, as Section \ref{sec:four_flavors} counts them.  The routines take all three---$\delta_{\rm CP}$, $\delta_{14}$ and $\delta_{24}$---and the last two default to zero, which is what the listing above leaves them at; \ref{app:sample_2nu_hamiltonians} gives the rotation ordering they enter through.
...
{\tt NuOscProbExact} is useful in part because it computes oscillation probabilities for an arbitrary time- or position-independent Hamiltonian.  Listing \ref{lst:3nu_arbitrary} shows a code template to compute three-neutrino oscillation probabilities using a user-supplied Hamiltonian that is added to the Hamiltonian for oscillations in matter.
...
Three habits follow from all of this.  First, pass arrays rather than looping, since that is worth more than any other single choice.  Second, build the energy-independent part of the vacuum Hamiltonian once and reuse it---the matter, NSI, and Lorentz-violating builders all take it as their first argument, at two, three, and four flavors---rather than rebuilding it inside a scan.
...
{\tt NuOscProbExact} offers the combination: flexibility, speed, and accuracy at once.  Flexibility and speed can each be found elsewhere, though not in one code.  Three of them reach round-off at constant density, each for the standard three-flavor case alone; reaching it for an arbitrary Hamiltonian takes diagonalizing numerically, at the cost in speed this construction was built to avoid.
...
\textit{Accuracy.---}At constant density, the answer given by {\tt NuOscProbExact} is exact to round-off.  Three of the other codes are equally exact there, each for the standard three-flavor case alone, and all three are faster; what distinguishes this one is that the same statement holds for every Hamiltonian in scope.  Through the Earth, where every code, this one included, must discretize the continuously varying profile, its error is the smallest on the plane at every setting down to $10^{-7}$, and past that {\tt nuSQuIDS} is finer.  Section \ref{sec:precision} gives both values and says what limits each.
...
So far we have compared accuracy at constant density.  Through the Earth, the density varies continuously along the trajectory.  The set of codes that can be compared changes with the problem: {\tt NuFast-LBL} and the expansion are limited to constant density, and {\tt nuCraft} has no constant-density mode; {\tt NuFast-Earth} has one, but is an Earth propagator and is measured here instead.
...
Each integrator was being handed a rescaled density, chosen to imitate a reference held in this library's constants.  Rescaling a density is not an exact substitute for changing a matter constant, because a code applies its own electron-fraction and mass handling to whatever it is given.  The rescaling left behind a small fixed offset, immune to every setting because it was never a numerical error to begin with.  Measured instead against a reference built in each code's own constants, the integrators descend as far as the codes that cut, and further: {\tt nuSQuIDS} reaches $1 \cdot 10^{-10}$ on the chord of \figu{planes}, where it had appeared to stop near $10^{-6}$.
...
\begin{figure*}[t!]
 ...
 \caption{\label{fig:planes}What accuracy costs, in three problems that ask progressively more of a code.  All three share one axis of time per probability, so that a cost can be carried by eye from one panel to the next.  {\it Top}: the $\nu_\mu$ row at $L = 1300$~km through constant matter of $3$~g~cm$^{-3}$, largest error over its three channels and over $60$ energies between $0.6$ and $20$~GeV.  The second-order expansion is the one entry scored on $P_{\nu_\mu \to \nu_e}$ alone, since that is the only channel it returns.  Nothing is approximated there, so the reference is exact: the same Hamiltonian exponentiated in $50$-digit arithmetic.  {\it Middle}: the same row along the PREM chord at $\cos\theta_z = -0.9$, a baseline of $11468$~km, over twelve energies between $3$ and $40$~GeV.  {\it Bottom}: the same chord with a sterile state, at the mixing of \figu{sterile_earth}, $\Delta m_{41}^2 = 1$~eV$^2$ and $\sin^2\theta_{14} = \sin^2\theta_{24} = 0.1$, over twelve energies between $0.3$ and $30$~TeV, on the $\nu_\mu \to \nu_\mu$ channel alone; {\tt NuFast-Earth}, {\tt GLoBES}, and {\tt Prob3++} hard-wire three flavors and cannot run it.  In the two lower panels the density varies along the chord and every code, this one included, must approximate the profile, so no exact answer exists and the reference is instead a converged one, described in the text.  {\tt NuOscProbExact} appears twice in the lower two panels, once for each dial it exposes, and {\tt NuFast-Earth} twice for a different reason; the text gives both, along with the constants each code is allowed to keep.  Each code is driven through whatever entry point its own interface offers, which for a cost per probability is the first thing that separates them: this code, {\tt NuFast-Earth} and {\tt nuSQuIDS} take a whole array of energies in one call; {\tt GLoBES} and {\tt Prob3++} expose no such entry point and are looped over; and {\tt nuCraft} accepts a list of particles but loops over it internally, so what it costs is a loop's.  Threads are likewise whatever each code uses unbidden.  The four compiled binaries link no threading library and run on one; this code reaches the machine's twelve through its compiled kernel; and for {\tt nuSQuIDS} and {\tt nuCraft}, both driven from {\tt Python}, the count was neither fixed nor recorded, as Section \ref{sec:performance} sets out.  The costs plotted are per probability at the settings drawn, which run far finer than a figure such as \figu{sterile_earth} asks for, so they are not the cost of producing one.  Lower and further left is better.}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


Figure \ref{fig:planes} shows what accuracy costs each code.  Most of these codes expose a setting that trades accuracy for time, so a comparison made at one choice of that setting would say as much about the choice as about the code.  Each setting is therefore traced through its range.  The three panels share one axis of time per probability, so that a cost can be carried by eye from one problem to the next; they do not share a set of codes, because not every code can be posed every problem.  Throughout, each code keeps its own $\hbar c$ and its own normalization of $V_{\rm CC}$, and the reference it is measured against is built in those same constants, so that no code is charged for a constant it defines differently.

\smallskip

\textit{Constant density.---}In constant matter nothing is approximated.  The density enters the Hamiltonian as a number, the closed forms are exact, and what is left is floating-point arithmetic; the reference is correspondingly exact, the same Hamiltonian exponentiated in $50$-digit arithmetic, and it agrees with itself to $3 \cdot 10^{-17}$.  Four of the six entries in the top panel sit at round-off and are separated only by speed: {\tt NuFast-LBL} at $8 \cdot 10^{-16}$ in $0.07$~$\mu$s, {\tt Prob3++} at $1 \cdot 10^{-15}$ in $0.5$~$\mu$s, {\tt GLoBES} at $6 \cdot 10^{-15}$ in $0.4$~$\mu$s, and {\tt NuOscProbExact} at $8 \cdot 10^{-15}$ in $1$~$\mu$s.  None of the four has an accuracy setting here, because none of them approximates anything for a setting to refine.

The other two entries do not sit there, for opposite reasons.  {\tt nuSQuIDS} integrates an ordinary differential equation where the others exponentiate, and its tolerance is a real dial: it carries the code from $9 \cdot 10^{-4}$ at $24$~$\mu$s to $4 \cdot 10^{-12}$ at $98$~$\mu$s, four times the time for eight orders of magnitude, with no floor inside its range.  The second-order expansion in $\alpha$ and $s_{13}$ costs $0.3$~$\mu$s and sits at $5 \cdot 10^{-3}$, twelve orders further out than the codes around it, and no setting moves it: what limits it is the truncation of a series rather than any arithmetic.
...
Down to about $10^{-7}$, {\tt NuOscProbExact} is the cheapest route to any given accuracy on the plane, setting aside {\tt NuFast-Earth}'s cached $\delta_{\rm CP}$ scan, which is drawn beside it and taken up below.  It reaches $10^{-3}$ in $9$~$\mu$s where {\tt NuFast-Earth}, the next cheapest, needs $26$; $10^{-4}$ in $20$~$\mu$s against $109$; and $10^{-6}$ in $120$~$\mu$s where {\tt nuSQuIDS} needs $605$ and either {\tt GLoBES} or {\tt NuFast-Earth} about $2000$.  The margin is a factor of $3$ at the loose end and $5$ at the tight one.
...
Table \ref{tab:codes} shows what each of these codes was built to do and what its interface accepts.  Those two facts decide more code choices than speed or accuracy do: a code that cannot express the problem at hand is not a candidate at any speed.  Speed and accuracy only ever rank the candidates that remain.
...
Most of them fix what may be put in the Hamiltonian as firmly as they fix the flavor count.  Most of these codes take oscillation parameters---angles, phases, mass splittings---and build the Hamiltonian themselves.  That design is convenient, because the user supplies familiar parameters, and confining, because only the terms its authors built in can ever appear.  {\tt nuSQuIDS} and {\tt NuOscProbExact} take the Hamiltonian itself: a new oscillation scenario is something the user writes down, not a modification of the source.  
...
Set beside existing oscillation codes (Section \ref{sec:comparison}), {\tt NuOscProbExact} makes good on three claims: flexibility, speed, and accuracy.  The call accepts the Hamiltonian itself, any Hermitian matrix, rather than a list of oscillation parameters, so a non-standard interaction, a Lorentz-violating term, an added sterile state, all three at once, or a completely novel oscillation scenario is the same computation as the standard case.  Among the six codes we examined that cross the Earth, and leaving aside a scan short enough for one of them to answer from a cache, {\tt NuOscProbExact} is the quickest route to every accuracy down to about $10^{-7}$, past which {\tt nuSQuIDS} is both finer and faster (the middle panel of \figu{planes}).  A scan requested as one call comes out two orders of magnitude cheaper per probability than the same scan run point by point (\figu{performance}).  At constant density its answer differs from a $50$-digit reference by round-off alone, as do those of the other exact codes there, which are faster (the top panel of \figu{planes}); no accuracy setting exists there because no approximation is made for one to refine.
...
Table \ref{tab:tests} lists what the 904 tests of {\tt NuOscProbExact} cover; Section \ref{sec:structure} shows the modules they exercise.  They reach every line and every branch of the library: coverage is $100\%$ in v1.13.1, held above a floor of $98\%$ that fails the build if it drops.

The tests run on every push and every pull request from the GitHub repository, so a contributor need not invoke them.  They are also the quickest way to confirm a local installation.  The tests are not packaged with the library, but they run from a clone:
\begin{verbatim}
pip install -e ".[test]"
pytest
\end{verbatim}
...
Three of them carry more of this paper than the others.  Every figure in this paper is produced by notebook \#10: the probabilities in them are computed as the notebook runs, except in the figures that compare this code against another, where every code's numbers, including this one's, are read from data frozen in the repository.  Notebook \#17 works through the external corroboration summarized in Section \ref{sec:validation}: what {\tt nuSQuIDS} and the Zaglauer--Schwarzer closed form each agree with, to what accuracy, and---the laborious part---which conventions had to be matched before either comparison meant anything.  Notebook \#20 is where the long-range potential of Section \ref{sec:arbitrary_hamiltonian} is built and validated, Listing \ref{lst:3nu_arbitrary} being the core of it.
...
Neither of these last two Hamiltonians is drawn on in this paper.  Every four-flavor figure in it rests on $\mathbb{H}_4^{\rm matt}$ instead: the four-flavor case of \figu{validation}, at constant density, and \figu{sterile_earth} and the bottom panel of \figu{planes}, slab by slab through PREM, the first of those two with the antineutrino exchange of \equ{antineutrino}.  The suite of \ref{app:tests} exercises the NSI and the Lorentz-violating Hamiltonians, and notebook \#16 of \ref{app:notebooks} works through the NSI one.
