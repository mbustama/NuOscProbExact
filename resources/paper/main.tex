...
Each code runs with the parallelism it has, and none is pinned to one thread: the four compiled codes link no threading library and run serially, while {\tt NuOscProbExact} reaches the twelve threads of the machine through its compiled kernel.  Every ratio between this code and another therefore sets a parallel implementation against serial ones, which is what an ordinary installation of each provides rather than a measure of the arithmetic alone.  On the constant-density plane of \figu{planes} a single-threaded series is drawn beside the parallel one, and the two differ by less than a part in $10^4$; along the Earth chord, where the kernel has more work to divide, only the parallel series is drawn.  The two figures also differ in how a time is taken: \figu{performance} reports the best of repeated runs for this library and a cold run for each external code, while \figu{planes} reports the mean of an autoranged block for every code alike.  Absolute times therefore mean little on their own and would be perhaps a factor of two different on a workstation; the ratios between codes, which is what these figures are for, are far more stable than that.
